...
if MAG_SUPPORT
    ...
    %% Alignment
    if Status == Alignment && StaticFlag == 1
        gyro_bias(1) = mean(alignGyroArray(:, 1));
        gyro_bias(2) = mean(alignGyroArray(:, 2));
        gyro_bias(3) = mean(alignGyroArray(:, 3));
        % g~ in body frame is [g_x, g_y, g_z], and g~b = Cnb*[0, 0, g]
        g_x = -mean(alignAccArray(:,1));
        g_y = -mean(alignAccArray(:,2));
        g_z = -mean(alignAccArray(:,3));
        if MAG_SUPPORT
            m_x = 0;
            m_y = 0;
            m_z = 0;
            ...
            for j = 1:magCount-1
                m_count = m_count + 1;
                m_x = m_x + alignMagArray(j, 1);
                m_y = m_y + alignMagArray(j, 2);
                m_z = m_z + alignMagArray(j, 3);
            end
            m_x = m_x / m_count - mag_bias(1);
            m_y = m_y / m_count - mag_bias(2);
            m_z = m_z / m_count - mag_bias(3);
